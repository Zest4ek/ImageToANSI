...
def _import():
    ...
    if file_path: 
        ...
        if os.path.getsize(file_path) == 0: # check if file is empty
            raise ValueError("\033[38;5;1mError: You have opened an empty file.")
        if not set(content).issubset(set("0123456789ABCDEF/")): # .ansi file can contain ONLY these characters
            raise ValueError("\033[38;5;1mError: You opened a file, which not supported by the program.\033[0m")

        # set console size
        lines = content.split("/")
        cols_con = int(len(lines[0]))
        lines_con = len(lines)+1
        try: subprocess.run(f"mode con: cols={cols_con} lines={lines_con}", shell=True, check=True)
        ...
        for line in lines:
            ...
            for i in range(0, len(line), 2):
                index = line[i:i+2]
                row.append(f"\033[38;5;{int(index,16)}m██") # convert hexadecimal to decimal index
            print("".join(row))
    else: 
        raise ValueError("\033[38;5;1mFile not selected.\033[0m")
...
def _export():
    ...
    os.system(f"mode con: cols=120 lines=30") # set default console size
    ...
    # calculate ANSI colors
    for y in range(img.height):
        for x in range(img.width):
            r,g,b = img_col[x,y]
            r,g,b = min(5,int(r/256*6)),min(5,int(g/256*6)),min(5,int(b/256*6))
            index = 16+36*r+6*g+b
            ansi.append(f"{index:X}") # convert decimal index to hexadecimal
        ansi.append("/")
    ansi.pop() # removing an extra slash
            
    if not os.path.exists("Saves"):
        os.makedirs("Saves")    
    with open(f"Saves/{name}.ansi", "w", encoding="utf-8") as file:
        file.write("".join(ansi))
    os.system("cls||clear")
...
def _print():
    ...
    # calculate and print ANSI colors
    for y in range(img.height):
        row = []
        for x in range(img.width):
            r,g,b = img_col[x,y]
...
def _compress():
    ...
    file_path = filedialog.askopenfilename(title="Select an image",
                                           filetypes=(("Image file", "*.png *.jpg *.jpeg *.ico *.gif"), ("Another image file", "*.*")))
    if file_path: 
        try:
    ...
    if not inp.isdigit():
        if inp == "":
            ...
            if img.width > img.height:
                ...
                compress_num = (img.height / screen_height)*20
        else:
            raise ValueError("\033[38;5;1mError: Incorrect input.\033[0m")
    else:
        # set to custom
        compress_num = int(inp)
        if int(compress_num) <= 0:
            raise ValueError("\033[38;5;1mError: Incorrect input.\033[0m")
    try:
        ...
        # set console size
        subprocess.run(f"mode con: cols={int(img.width/compress_num*2)} lines={int(img.height/compress_num)+1}", shell=True, check=True) 
    except subprocess.CalledProcessError: pass
    except Exception:
        raise ValueError("\033[38;5;1mError: The image is too small or the compression ratio is too high.\033[0m")
...
def main():
    ...
    print("\033[38;5;3m# \033[38;5;11mImage to ANSI by Zest4ek\033[0m\n", 
          "\033[38;5;10m.print\033[0m   :: print an image\n",
          "\033[38;5;10m.import\033[0m  :: import .ansi image\n",
